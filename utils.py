...
def movies_by_rating(rating):
    rating_parameters = {"children": "'G'", "family": "'G', 'PG', 'PV-13'", "adult": "'R', 'NC-17'"}
    if rating not in rating_parameters:
        return f"Переданной группы не существует"
...
def movies_by_genre(genre):
    result = execute_query(f"""select title,description,listed_in 
           from netflix
...
def cast_partners(actor1, actor2):
    query = f"select `cast` from netflix where `cast` like '%{actor1}%' and  `cast` like '%{actor1}%';"
    result = execute_query(query)
    actor_list = []
    for cast in result:
